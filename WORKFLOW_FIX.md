...
Добавлена **умная проверка** контейнеров вместо слепого пересоздания:

```bash
# Проверяем существует ли контейнер
if docker ps -a --format '{{.Names}}' | grep -q '^nfc_postgres$'; then
  # Если существует но остановлен - запускаем
  if ! docker ps --format '{{.Names}}' | grep -q '^nfc_postgres$'; then
    docker start nfc_postgres
  else
...
**Почему важно:**
- ✅ **Проверяет существование** контейнера перед действием
- ✅ **Запускает** существующий остановленный контейнер через `docker start`
- ✅ **Создаёт** только если контейнер не существует
- ✅ **Никогда не пересоздаёт** (данные БД в безопасности!)
- ❌ **Не падает** с "container already in use"
